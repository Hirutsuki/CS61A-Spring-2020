...
def link_to_list(link):
    ...
    while link is not Link.empty:
        if not isinstance(link.first, Link):
            ...
            link_list.append(link_to_list(link.first))
        link = link.rest
    return link_list
...
def cumulative_mul(t):
    ...
    def f(t):
        ...
        sub_labels = [f(b) for b in t.branches]
        sub_product = 1
        for label in sub_labels:
            sub_product *= label

        t.label = t.label*sub_product
        return t.label
...
def reverse_other(t):
    ...
    for i in range(len(t.branches)//2):
        t.branches[i].label, t.branches[-(i+1)].label = t.branches[-(i+1)
                                                                   ].label, t.branches[i].label
    for odd_branches in [b.branches for b in t.branches]:
