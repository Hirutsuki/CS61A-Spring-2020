...
def distance(city1, city2):
    ...
    "*** YOUR CODE HERE ***"
    def square(fn):
        return pow(fn(city1)-fn(city2), 2)
    return sqrt(square(get_lat)+square(get_lon))
...
def closer_city(lat, lon, city1, city2):
    ...
    """
    "*** YOUR CODE HERE ***"
    closer = min([city1, city2], key=lambda city: distance(
        make_city('', lat, lon), city))
...
def add_trees(t1, t2):
    ...
    if len2 == 0:
        return t1
    if len1 < len2:
        branches1 = branches(t1)+[tree(0) for _ in range(len(t2)-len(t1))]
        return add_trees(tree(label(t1), branches1), t2)
